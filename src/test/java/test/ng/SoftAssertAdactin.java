...
public class SoftAssertAdactin extends BaseClass{
...
//		launchUrl("http://adactinhotelapp.com/");
		s.assertTrue(getCurrentUrl().contains("adactin"));
		sendKeys(h.getUsername(), "jothikrishna");
		s.assertEquals(getAttribute(h.getUsername()),"jothikrishna","verify username");
		sendKeys(h.getPass(), "Life@heaven");
		s.assertEquals(getAttribute(h.getPass()),"Life@heaven","verify password");
		click(h.getLogin());
		Thread.sleep(3000);	
	...
	private void test2() throws InterruptedException {
		...
		s.assertFalse(h1.getLocation().isSelected(),"selected or not");
		selectByIndex(h1.getHotel(), 2);
		selectByIndex(h1.getRoom(), 2);
		selectByIndex(h1.getRoomcount(), 1);
		clear(h1.getCheckin());
		sendKeys(h1.getCheckin(), "08/01/2022");
		s.assertEquals(getAttribute(h1.getCheckin()), "08/01/2022","verify checkin date" );
		clear(h1.getCheckout());
		sendKeys(h1.getCheckout(), "09/01/2022");
		s.assertNotEquals(getAttribute(h1.getCheckout()), "10/01/2022","verify checkout date" );
	...
	private void test4() throws InterruptedException {
		...
		s.assertEquals(getAttribute(h3.getAddress()), "no.10,kkk nagar","Address");
